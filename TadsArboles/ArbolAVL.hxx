...
template <class T>
void ArbolAVL<T>::inOrdenHasta(NodoAVL<T> *inicio,list< pair < T, int> > &l, int tam, int nivel)
{
    if (inicio == NULL)
      return;
    inOrdenHasta(inicio->obtenerHijoIzq(),l,tam,nivel+1);
    //cout << inicio->obtenerDato() << " ";
    if(l.size()< tam)
    l.push_back(make_pair(inicio->obtenerDato(),nivel));
    inOrdenHasta(inicio->obtenerHijoDer(),l,tam,nivel+1);
}
template <class T>
